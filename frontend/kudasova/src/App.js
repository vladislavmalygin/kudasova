...
const PhotoPage = () => {
    ...
    useEffect(() => {
        const fetchImages = async () => {
            try {
                const response = await fetch('http://0.0.0.0:8080/api/images/');
                if (!response.ok) {
...
const VideoPage = () => {
    ...
    useEffect(() => {
        const fetchVideos = async () => {
            const response = await fetch('http://0.0.0.0:8080/api/videos/');
            const data = await response.json();
            setVideos(data);
        };
...
const MemoriesPage = () => {
    ...
    useEffect(() => {
        const fetchMemories = async () => {
            try {
                const response = await fetch('http://0.0.0.0:8080/api/memories/');
                const data = await response.json();
                setMemories(data);
            } catch (error) {
...
const GuestbookForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://0.0.0.0:8080/api/guestbook/', {
                method: 'POST',
                headers: {
...
const PoemsPage = () => {
    ...
    useEffect(() => {
        const fetchPoems = async () => {
            try {
                const response = await fetch('http://0.0.0.0:8080/api/poems/');
                const data = await response.json();
                setPoems(data);
            } catch (error) {
                console.error('Ошибка при получении воспоминаний:', error);
